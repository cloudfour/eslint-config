...
function loadESLintIgnoreFile(filePath) {
  ...
  try {
    ...
    error.message = `Cannot read .eslintignore file: ${filePath}\nError: ${error.message}`;
    throw error;
  }
}
...
function createContext(
  ...
  const matchBasePath =
    providedMatchBasePath || (filePath && path.dirname(filePath)) || cwd;
...
class ConfigArrayFactory {
  ...
  *_normalizeObjectConfigDataBody(
    ...
    // Flatten `overries`.
    for (const [i, element] of overrideList.entries()) {
      yield* this._normalizeObjectConfigData(element, {
        ...ctx,
