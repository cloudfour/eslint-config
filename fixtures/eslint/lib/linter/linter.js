...
function createDisableDirectives(options) {
  ...
  const directiveRules = ruleIds.length > 0 ? ruleIds : [null];
  const result = {
...
function resolveParserOptions(
  ...
  const mergedParserOptions = lodash.merge(
  ...
  _verifyWithoutProcessors(textOrSourceCode, providedConfig, providedOptions) {
    ...
    const envInFile =
      options.allowInlineConfig && !options.warnInlineConfig
        ? findEslintEnv(text)
        : {};
    const resolvedEnvConfig = { builtin: true, ...config.env, ...envInFile };
    ...
    } else if (!slots.lastSourceCode.scopeManager) {
      /*
    ...
    try {
      ...
      if (error.currentNode) {
        const { line } = error.currentNode.loc.start;
  ...
  _verifyWithProcessor(textOrSourceCode, config, options, configForRecursive) {
    ...
    const messageLists = preprocess(text, filenameToExpose).map((block, i) => {
      ...
      if (configForRecursive && path.extname(blockName) !== originalExtname) {
        debug(
          'Resolving configuration again because the file extension was changed.'
        );
